...
- **Task soft-delete via `tasks.archived_at` sejak Fase 5** (migrasi `20260908010000_task_archive.sql`, kolom nullable + partial index `tasks_active_by_list_idx WHERE archived_at IS NULL`, **belum di-apply ke DB live**). `useTasks.ts` filter `archived_at IS NULL` untuk board aktif; `useArchivedTasks.ts` query terpisah untuk task ter-archive. `useArchiveTask` optimistic (satu-satunya mutation baru Fase 5 yang optimistic — Duplicate/Restore/Delete-permanen sengaja tidak, tunggu konfirmasi server sebelum update UI).
...
- Labels, assignee, checklist, task-card ornaments — **digeser ke Fase 6** (semula ditulis sebagai bagian "Fase 5" di catatan lama, direvisi karena Fase 5 aktual difokuskan ke task actions + dialog upgrade)
